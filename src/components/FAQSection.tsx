  ...
  <div className="container mx-auto px-4 md:px-6">
    ...
    <div className="bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-2xl p-8">
      <div className="grid lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="space-y-4">
          <button onClick={() => setIsContactModalOpen(true)}  className="group w-full inline-flex items-center justify-center gap-3 px-6 py-4 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-all duration-300">
    ...
    <div className="mt-12 grid md:grid-cols-3 gap-6">
      {[
        ...
        <div 
          key={index}
          className="bg-white border border-slate-200 rounded-xl p-5 text-center hover:border-slate-300 transition-colors duration-300"
        >
          <div className={`w-12 h-12 ${getColorClass(item.color).bg} rounded-xl flex items-center justify-center mx-auto mb-4`}>
            <item.icon className={`w-6 h-6 ${getColorClass(item.color).text}`} />
          </div>
          <h4 className="font-bold text-slate-900 mb-1">{item.title}</h4>
          <p className="text-sm text-slate-600 mb-3">{item.description}</p>
          <div className="text-sm font-medium text-slate-900">{item.contact}</div>
        </div>
      ))}
    </div>
  </div>
